...
class ConfigHeader(QObject):
    loginCookiesFolder = 'cookies/headers/loginInfor.cks'  # 设置登录缓存文件路径
...
class ConfigSearchArea(QObject):
    ...
    def __init__(self, searchArea):
        ...
        self.currentIndex = 0  # 初始化最近索引
        # current widgets name。
        self.currentName = '网易云'  # 最近搜索网站名

        # parent.
        self.searchArea = searchArea  # 搜索区
...
class ConfigDetailSings(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.detailSings = parent  # 初始化歌曲详情
        self.musicList = []  # 初始化音乐列表
    ...
    # 绑定连接
    def bindConnect(self):
        self.detailSings.playAllButton.clicked.connect(self.addAllMusicToPlayer)  # 歌曲详情页，播放全部按钮，点击，连接（添加全部歌曲到歌单）
        self.detailSings.singsTable.itemDoubleClicked.connect(self.itemDoubleClickedEvent)  # 歌曲详情页，歌曲列表，双击，连接（项双击事件处理）
    ...
    # 项双击事件处理方法
    def itemDoubleClickedEvent(self):
        currentRow = self.detailSings.singsTable.currentRow()  # 最近行：歌曲详情页，歌曲列表，最近行
        data = self.musicList[currentRow]  # 数据：歌曲列表，最近行

        self.playList.setPlayerAndPlayList(data)  # 设置播放器和播放列表（数据）
...
class ConfigSystemTray(QObject):
    ...
    def __init__(self, systemTray):
        ...
        self.systemTray = systemTray  # 初始化系统托盘

        self.addActions()  # 初始化添加动作
        self.bindConnect()  # 初始化绑定连接

    # 添加动作
    def addActions(self):
        closeAction = QAction('退出', self.systemTray, triggered=self.systemTray.parent.close)  # 定义动作（关闭的槽函数）

        self.systemTray.addAction(closeAction)  # 执行关闭
