...
class OneSing(QFrame):
    # 大量创建，这样可以省内存。
    __slots__ = ('parent', 'ggparent', 'detailFrame', 'row', 'column', 'ids',
    ...
    clicked = pyqtSignal(str, str)  # QT信号实例化
    ...
    def __init__(self, row, column, ids=None, parent=None, picName=None):
        ...
        self.setMinimumSize(180, 235)  # 设置最小尺寸

        self.picLabel = QLabel()  # 歌单图片标签
        self.picLabel.setObjectName('picLabel')  # 标签名称
        self.picLabel.setMinimumSize(180, 180)  # 最小尺寸
        self.picLabel.setMaximumSize(180, 180)  # 最大尺寸

        self.nameLabel = QLabel()  # 名称标签
        self.nameLabel.setMaximumWidth(180)  # 歌单名称最大宽度
        self.nameLabel.setWordWrap(True)  # 设置单词换行

        self.mainLayout = QVBoxLayout(self)  # 主布局：竖直盒子布局

        self.mainLayout.addWidget(self.picLabel)  # 主布局添加部件（图片标签）
        self.mainLayout.addWidget(self.nameLabel)  # 主布局添加部件（名称标签）

    # 设置页面样式。
    def setStyleSheets(self, styleSheet=None):
        if styleSheet:
            self.setStyleSheet(styleSheet)  # 设置页面样式
...
class PlaylistButton(QPushButton):
    ...
    """
    __slots__ = ('parent', 'grandparent', 'ids', 'coverImgUrl',
        'catch', 'detailFrame', 'result', 'singsIds', 'singsUrls'
        )

    hasClicked = pyqtSignal(int, str)  # QT信号实例化
    ...
    def __init__(self, parent, ids, coverImgUrl, *args):
        ...
        self.clicked.connect(self.clickedEvent)  # 点击，连接到：点击事件方法
