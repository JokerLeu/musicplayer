...
class PlayWidgets(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.setObjectName('playwidget')  # 名字
        self.parent = parent
        # 0模式是列表循环。1模式是单曲循环，2模式是随机播放。
        self.module = 0  # 默认播放模式

        with open('QSS/playWidgets.qss', 'r', encoding='utf-8') as f:
            self.setStyleSheet(f.read())  # 设置样式

        self.playList = PlayList(self)  # 初始化播放列表
        self.currentMusic = CurrentMusic(self)  # 初始化最近音乐
        self.player = Player(self)  # 初始化播放器

        # test desktopLyric
        self.desktopLyric = DesktopLyric(self)  # 初始化桌面歌词

        self.setButtons()  # 设置按钮
        self.setLabels()  # 设置歌曲时间
        self.setSliders()  # 设置进度条

        self.setLayouts()  # 设置布局

        self.loadCookies()  # 加载缓存
    ...
    def setPlayerAndPlayList(self, data:dict, index=0):
        ...
        authorAndName = data['author'] + data['name']  # 新加的：作者+歌名
        for i, mediaInfo in enumerate(self.playList.musicList):  # 枚举播放列表
            checkAuthorAndName = mediaInfo['author'] + mediaInfo['name']  # 已存在的：作者+歌名
            # if data in self.playList.musicList:
            if authorAndName == checkAuthorAndName:  # 新加==已存在
                # index = self.playList.musicList.index(data)
                if self.player.playList.currentIndex == i and i != 0:  # 最近序号==序号且序号非0
                    return

                self.player.setIndex(i)  # 设置索引到i
        ...
        sureSetUp = self.player.setMusic(data['url'], data)  # 设置当前歌曲（地址，数据）
        if not sureSetUp:  # 如果无法解析地址
...
class PlayList(QFrame):
    """播放列表。"""
    musicListCookiesFolder = 'cookies/playlist/musicList.cks'  # 音乐列表的缓存文件
    allCookiesFolder = [musicListCookiesFolder]  # 所有音乐列表缓存
    ...
    def __init__(self, parent=None):
        ...
        self.musicList = []  # 初始化歌曲列表

        self.currentRow = -1

        self.allRow = 0

        self.itemColor = QBrush(QColor.fromRgb(95, 95, 99))  # 项颜色： QT刷（颜色值）

        with open('QSS/playList.qss', 'r') as f:
            self.setStyleSheet(f.read())  # 设置样式表

        self.resize(574, 477)  # 初始化详情页尺寸

        self.hide()  # 默认隐藏

        self.setButtons()  # 初始化按钮
        self.setLabels()  # 初始化标签
        self.setTables()  # 初始化表格

        self.setLayouts()  # 初始化布局

    # 布局。
    # 初始化按钮
    def setButtons(self):
        self.closeButton = QPushButton("×", self)  # 创建关闭按钮
        self.closeButton.setObjectName("closeButton")  # 名字
        self.closeButton.clicked.connect(self.hide)  # 点击后连接（隐藏）
    ...
    def addMusic(self, data):
        self.musicList.append(data)  # 直接添加
    ...
    def saveCookies(self):
        with open(self.musicListCookiesFolder, 'wb') as f:  # 打开w写b二进制
            pickle.dump(self.musicList, f)  # 序列化

    # 加载缓存@检查文件夹
    @checkFolder(allCookiesFolder)
    def loadCookies(self):
        with open(self.musicListCookiesFolder, 'rb') as f:  # 打开r读b二进制
            self.musicList = pickle.load(f)  # 反序列化并加载

        for i in self.musicList:  # 枚举
            self.addPlayList(i['name'], i['author'], i['time'])  # 赋值
...
class CurrentMusicDetail(ScrollArea):
    ...
    def __init__(self, parent):
        ...
        self.lyricFrames = ScrollArea()  # 歌词框架（滚动条区）
        self.lyricFrames.setObjectName('lyricScroll')  # 名字
        self.lyricFrames.frame.setObjectName('lyricFrame')  # 歌词框架，框架名字
        self.lyricFramesLayout = VBoxLayout(self.lyricFrames.frame)  # 歌词布局：垂直布局（歌词框架，框架）
        # self.lyricFrames.setMaximumHeight(500)
        # 为歌词创建索引方便删除。
        self.allLyrics = []  # 初始化所有歌词
...
class Player(QMediaPlayer):
    ...
    def __init__(self, parent=None):
        ...
        self.transTime = addition.itv2time  # 转换后的时间mm:ss
        self.musicTime = 0
        self.playList = _MediaPlaylist(self)  # 播放列表
        # 默认列表循环。
    ...
    def setMusic(self, url, data):
        """设置当前的音乐，可以直接用网络链接。"""
        if url:  # 设置了地址
            if 'http' in url or 'file' in url:  # 带前缀标记
                self.playList.addMedias(QMediaContent(QUrl(url)), data)  # 按照链接处理
            else:
                self.playList.addMedias(QMediaContent(QUrl.fromLocalFile(url)), data)  # 按照本地文件处理
            return True
        return False
    ...
    def playMusic(self, url=None, data=None):
        """播放音乐。"""
        if url:
            self.setMusic(url, data)  # 设置音乐
            # 播放事件。
            self.playWidgets.playEvent(self)  # 播放部件，播放事件
        else:
            self.playWidgets.playEvent(self)  # 播放部件，播放事件
...
class _TableWidget(QTableWidget):
    ...
    def __init__(self,parent=None):
        ...
        self.entered.connect(self.itemHover)  # 进入，连接（鼠标悬停处理）
        self.viewportEntered.connect(self.viewportHover)  # 视口输入，连接（非悬停还原处理）
        # #2D2F33
...
class _MediaPlaylist(QObject):
    musicsCookiesFolder = 'cookies/mediaPlaylist/musics.cks'  # 音乐缓存
    mediaListCookiesFolder = 'cookies/mediaPlaylist/mediaList.cks'  # 音乐列表缓存
    allCookiesFolder = [musicsCookiesFolder, mediaListCookiesFolder]  # 全部缓存文件夹
    ...
    def __init__(self, parent=None):
        ...
        # currentModel
        # 默认是3.循环。
        self.myModel = 3  # 播放模式序号

    # 设置初始化连接
    def setInitConnection(self):
        self.parent.mediaStatusChanged.connect(self.mediaStatusChangedEvent)  # 音乐播放状态改变，连接（音乐播放状态改变事件处理）
    ...
    def addMedias(self, url, data):
        ...
        self.currentIndex = len(self.musics) - 1  # 最近索引到最后
    ...
    def clear(self):
        self.musics = []  # 音乐列表清空
        self.mediaList = {}  # 媒体字典清空
        self.currentIndex = 0  # 最近序号归零
        self.duration = 0  # 持续时间归位

    # 歌曲列表计数方法
    def mediaCount(self):
        return len(self.musics)  # 返回歌曲列表长度
    ...
    # 下一曲方法
    def next(self):
        if self.myModel == 4:  # 模式4:随机播放
            index = random.randint(0, len(self.musics) - 1)  # 随机列表内取值
            self.setCurrentIndex(index)  # 最近索引赋值
            return

        if self.currentIndex + 1 >= len(self.musics):  # 非随机：超过列表
            self.currentIndex = 0  # 返回第一项
        else:
            self.currentIndex += 1  # 加1

        self.play()  # 调用播放方法

    # 上一曲方法
    def previous(self):
        if self.myModel == 4:  # 随机
            index = random.randint(0, len(self.musics) - 1)
            self.setCurrentIndex(index)
            return

        if self.currentIndex - 1 <= -1:  # 非随机：小于列表
            self.currentIndex = 0  # 返回第一项
        else:
            self.currentIndex -= 1  # 减1
    ...
    def play(self):
        try:
            media = self.musics[self.currentIndex]  # 尝试令当前歌曲为最近索引的歌曲
        except:
            logger.error("unknow error. musics info: {0}".format(self.musics))  # error：最近索引
            return

        if type(media) == str:
            if 'http' in media or 'file' in media:  # 包含前缀
                content = QMediaContent(QUrl(media))  # 解析地址
            else:
                content = QMediaContent(QUrl.fromLocalFile(media))  # 不包含前缀：解析本地文件

            self.musics = self.musics[:self.currentIndex] + [content] + self.musics[self.currentIndex + 1:]
            media = content

        self.parent.setMedia(media)  # 父表
        self.parent.playMusic()
