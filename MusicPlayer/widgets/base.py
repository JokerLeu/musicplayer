...
class ScrollArea(QScrollArea):
    ...
    scrollDown = pyqtSignal()  # 实例化QT信号

    def __init__(self, parent=None):
        super(ScrollArea, self).__init__()  # 初始化
        self.parent = parent
        self.frame = QFrame()  #QT框架
        self.frame.setObjectName('frame')  # 框架名称
        # 用于发出scroll滑到最底部的信号。
        self.verticalScrollBar().valueChanged.connect(self.sliderPostionEvent)  # 垂直滚动条，值改变，连接，滑块位置事件

        self.setWidgetResizable(True)  # 部件大小可更改性

        self.setWidget(self.frame)  # 设置部件，框架
    ...
    def sliderPostionEvent(self):
        if self.verticalScrollBar().value() == self.verticalScrollBar().maximum():  # 滑块到底
            self.scrollDown.emit()  # 启动了一个函数？
